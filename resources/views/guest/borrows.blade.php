...
<div class="page-header d-flex justify-content-between align-items-center flex-wrap gap-2 mb-4">
    <div>
        ...
        <p class="text-muted mb-0">Fine = ₱10 × overdue days × number of books.</p>
    </div>
    <a href="{{ route('guest.borrow.create') }}" class="btn btn-primary"><i class="bi bi-bookmark-plus me-1"></i>Borrow Books</a>
</div>
            ...
            <table class="table table-hover mb-0">
                ...
                <tbody>
                    @forelse($borrows as $borrow)
                        @php $outstanding = $borrow->borrowItems->where('returned_at', null)->count(); @endphp
                        <tr>
                            ...
                            <td>
                                @if($outstanding > 0)
                                    <a href="{{ route('guest.return.form', $borrow) }}" class="btn btn-sm btn-success"><i class="bi bi-arrow-down-left-square me-1"></i>Return books</a>
                                @else
                                    <span class="text-muted">—</span>
                                @endif
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="text-center py-5 text-muted">You have no borrows yet. <a href="{{ route('guest.borrow.create') }}">Borrow books</a></td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
